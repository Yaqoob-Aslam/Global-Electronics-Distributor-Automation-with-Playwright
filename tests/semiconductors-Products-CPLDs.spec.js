...
test.describe.serial('Enrgtech - Global Electronics Distributor', () => {
  ...
  async function completeCheckoutProcess(productNumber, currentProductUrl) {
    try {
      ...
      // NEW: Select Pakistan phone code from dropdown
      console.log('\n--- Selecting Pakistan Phone Code ---');
      try {
        // Method 1: Try clicking the flag dropdown first
        const flagDropdown = page.locator('.iti__selected-flag');
        if (await flagDropdown.isVisible({ timeout: 3000 })) {
          console.log('✓ Found phone flag dropdown');
          
          // Click to open dropdown
          await flagDropdown.click();
          console.log('✓ Clicked flag dropdown to open options');
          await page.waitForTimeout(1000);
          
          // Try to find and click Pakistan option
          const pakistanOptions = [
            page.locator("//span[@class='iti__country-name' and contains(text(), 'Pakistan')]"),
            page.locator('.iti__country-name').filter({ hasText: 'Pakistan' }),
            page.getByText('Pakistan', { exact: true }),
            page.locator('[data-country-code="pk"]'),
            page.locator('[data-country-code="PK"]')
          ];
          
          let pakistanSelected = false;
          for (const pakistanOption of pakistanOptions) {
            try {
              if (await pakistanOption.isVisible({ timeout: 2000 })) {
                await pakistanOption.click();
                console.log('✓ Selected Pakistan phone code from dropdown');
                pakistanSelected = true;
                break;
              }
            } catch {
              continue;
          ...
          if (!pakistanSelected) {
            console.log('⚠ Could not find Pakistan option in dropdown, trying alternative...');
            
            // Try JavaScript to select Pakistan
            await page.evaluate(() => {
              const dropdown = document.querySelector('.iti__selected-flag');
              if (dropdown) {
                // Find Pakistan option by data attributes
                const pkOption = document.querySelector('[data-country-code="pk"], [data-country-code="PK"]');
                if (pkOption) {
                  pkOption.click();
                }
              }
            });
            console.log('✓ Attempted Pakistan selection via JavaScript');
          }
          ...
          console.log('⚠ Phone flag dropdown not found');
          
          // Alternative: Look for other phone code selectors
          const phoneCodeSelectors = [
            page.locator('select[name="phone_code"]'),
            page.locator('[name="phone_code"]'),
            '#phone_code',
            'select#phone_code'
          ];
          ...
          for (const selector of phoneCodeSelectors) {
            try {
              if (await selector.isVisible({ timeout: 2000 })) {
                console.log(`Found phone code selector: ${selector}`);
                await selector.selectOption({ label: /Pakistan|PK/i });
                console.log('✓ Selected Pakistan phone code from select');
                break;
              }
            } catch {
              continue;
        ...
        console.log('Phone code selection error:', error.message);
      }

      // Fill mobile number
      await fillFieldWithScroll('mobile number', '+923023738608', [
        '#mobileNo',
        '[name="mobileNo"]',
        'input[name="mobileNo"]',
      ...
      await fillFieldWithScroll('PO number', randomPO, [  // CHANGED: Using random PO number
       ...
       // Select billing country
         // Select billing country - IMPROVED WITH SCROLL AND WAIT
      ...
      try {
        ...
        if (await billingCountry.isVisible({ timeout: 3000 })) {
          console.log('✓ Found billing country dropdown');
          
          // Step 1: First scroll to the element
          await billingCountry.scrollIntoViewIfNeeded();
          console.log('✓ Scrolled to billing country dropdown');
          await page.waitForTimeout(1000);
          
          // Step 2: Click to open dropdown
          await billingCountry.click();
          console.log('✓ Clicked billing country dropdown');
          await page.waitForTimeout(1500); // Increased wait time
          
          // Step 3: Try multiple methods to select Pakistan
          console.log('Attempting to select Pakistan...');
          
          // Method 1: Direct select using JavaScript (most reliable)
          try {
            await page.evaluate(() => {
              const select = document.querySelector('[name="billing_country"]');
              if (select) {
                // Try to find Pakistan option
                for (let i = 0; i < select.options.length; i++) {
                  if (select.options[i].text.trim() === 'Pakistan') {
                    select.selectedIndex = i;
                    select.dispatchEvent(new Event('change', { bubbles: true }));
                    console.log('Selected Pakistan via JavaScript (text match)');
                    return;
                  }
                }
                ...
                for (let i = 0; i < select.options.length; i++) {
                  if (select.options[i].text.toLowerCase().includes('pakistan')) {
                    select.selectedIndex = i;
                    select.dispatchEvent(new Event('change', { bubbles: true }));
                    console.log('Selected Pakistan via JavaScript (case-insensitive)');
                    return;
                  }
                }
                
                // Try by value containing 'pk'
                for (let i = 0; i < select.options.length; i++) {
                  if (select.options[i].value.toLowerCase().includes('pk')) {
                    select.selectedIndex = i;
                    select.dispatchEvent(new Event('change', { bubbles: true }));
                    console.log('Selected Pakistan via JavaScript (PK value)');
                    return;
                  }
                }
              }
            });
            console.log('✓ Attempted JavaScript selection');
          } catch (jsError) {
          ...
          // Method 2: Try Playwright select with exact text
          try {
            await billingCountry.selectOption({ label: 'Pakistan' });
            console.log('✓ Selected Pakistan by exact label');
          } catch (exactError) {
            console.log('Exact label selection failed:', exactError.message);
            
            // Method 3: Try case-insensitive match
            try {
              await billingCountry.selectOption({ label: /Pakistan/i });
              console.log('✓ Selected Pakistan by case-insensitive regex');
            } catch (regexError) {
              console.log('Regex selection failed:', regexError.message);
              
              // Method 4: Try by value PK
              try {
                await billingCountry.selectOption({ value: /pk/i });
                console.log('✓ Selected Pakistan by PK value');
              } catch (valueError) {
                console.log('Value selection failed:', valueError.message);
                
                // Method 5: Try the specific XPath option
                try {
                  const pakistanOption = page.locator("//select[@id='billing_country']/option[normalize-space(text())='Pakistan']");
                  if (await pakistanOption.isVisible({ timeout: 1000 })) {
                    const optionValue = await pakistanOption.getAttribute('value');
                    if (optionValue) {
                      await billingCountry.selectOption({ value: optionValue });
                      console.log(`✓ Selected Pakistan by XPath value: ${optionValue}`);
                    }
                  }
                } catch (xpathError) {
                  console.log('XPath selection failed:', xpathError.message);
                }
              }
            }
          }
          ...
          const selectedValue = await billingCountry.inputValue();
          console.log(`✓ Selected value: "${selectedValue}"`);
          
          // Get selected text
          const selectedOption = billingCountry.locator('option:checked');
          const selectedText = await selectedOption.textContent();
          console.log(`✓ Selected text: "${selectedText}"`);
          
          // Check if Pakistan was selected
          if (selectedText && selectedText.toLowerCase().includes('pakistan')) {
            console.log('✅ SUCCESS: Pakistan successfully selected as billing country');
          } else if (selectedValue && selectedValue.toLowerCase().includes('pk')) {
            console.log('✅ SUCCESS: Pakistan (PK) successfully selected as billing country');
          } else {
            console.log('⚠ WARNING: Pakistan may not have been selected');
            console.log(`   Current selection: Text="${selectedText}", Value="${selectedValue}"`);
            
            // Take screenshot for debugging
            try {
              await page.screenshot({ path: `debug-country-select-${productNumber}.png` });
          ...
          console.log('⚠ Billing country dropdown not visible');
        }
      } catch (error) {
        console.log('⚠ Could not select billing country:', error.message);
        
        // Take error screenshot
        try {
          await page.screenshot({ path: `error-country-select-${productNumber}.png` });
      ...
      // Additional wait after country selection
      await page.waitForTimeout(1000);
