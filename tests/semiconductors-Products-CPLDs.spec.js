          ...
          if (productUrl) {
            ...
            if (await safeNavigate(fullUrl)) {
              ...
              if (checkoutSuccess) {
                ...
                // After successful checkout and redirection, we're already on CPLDs page
                // Check if we need to go back to products list for next product
                if (i < testProducts - 1) {
                  console.log(`\n🔄 Preparing for next product test...`);
                  
                  // We're already on CPLDs page, wait for products to load
                  await page.waitForTimeout(3000);
                  
                  // Verify we're on CPLDs page
                  const currentUrl = await page.url();
                  if (currentUrl.includes('/cplds')) {
                    console.log('✓ Still on CPLDs page, ready for next product');
                  } else {
                    // If not on CPLDs page, navigate back
                    await safeNavigate('https://www.enrgtech.co.uk/category/semiconductors/programmable-logic-circuits/cplds');
                  }
                }
              }
